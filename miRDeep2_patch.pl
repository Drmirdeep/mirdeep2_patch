...
-F            Fast mode, don't check genome file and reads file for consistency

-T            Number of threads to use (only used for randfold right now)
...
my $tfile='miRDeep2.pl';

my $scripts=`which $tfile`;
$scripts =~ s/$tfile//;
$scripts =~ s/\s+//g;
...
getopts("a:b:cdt:uvq:s:z:r:p:g:EPFT:",\%options);
...
sub compute_randfold{
	...
	my $split=`split -l 2>&1`;
	my $nl=`wc -l $dir_tmp/precursors_for_randfold.fa`;
	if($nl =~ /\s*(\d+)/){
		$nl=$1;
	}else{
		$nl=0;
	...
	if($threads > 1 and $split =~ /usage/i and $nl){

		my $size=int($nl/$threads);
		$size++;
		$size++ if($size%2 != 0);
		my $ccwd=`pwd`;
		chomp $ccwd;
		chdir "$dir_tmp/";
		`split -a 10 -l $size precursors_for_randfold.fa`;
		open OUT,">thread_randfold.sh" or die "Could not create thread randfold file\n";
		my @F=<xaaaaaaa*>;
		print OUT "#!/bin/bash
		pids=\"\"
		";
		...
		foreach my $f(@F){
	    print OUT "randfold -s $f 99 > $f.rand &
		    pids=\"\$pids \$!\"
			";
		}
		print OUT"
		...
		`bash thread_randfold.sh`; 
		`cat xaaaaaa*.rand > precursors_for_randfold.rand`;	
		chdir $ccwd;
